		...
		if (type % 100 == type) {
			...
			switch (type) {
				case SC_CreateLock: {
					...
					if (existingLockID == -1) {
						NewServerRequest(serverRQs, name, SC_Server_CreateLock, inPktHdr->from, inMailHdr->from, 0, 0,
										 0);
				...
				case SC_CreateCondition: {
					...
					if (existingCVID == -1) {
						NewServerRequest(serverRQs, name, SC_Server_CreateCondition, inPktHdr->from, inMailHdr->from, 0,
										 0, 0);
					}
				...
				case SC_CreateMV: {
					...
					if (existingMVID == -1) {
						NewServerRequest(serverRQs, name, SC_Server_CreateMV, inPktHdr->from, inMailHdr->from, mvSiz, 0,
										 0);
					}
			...
			switch (type) {
				...
				case SC_Server_CreateCondition: {
					...
					//If doesn't, send a reply of NO to server that made the request
					if (existingCVID == -1) {
						sendReplyToServer(outPktHdr, outMailHdr, SC_ServerReply_CreateCondition, requestID, machineID,
										  mailbox, 0);
					}
				...
				case SC_Server_DestroyCondition: {
					...
					//If it's not in our indexes, we don't have it, so reply no
					if (cvNum / 100 != myMachineID) {
						sendReplyToServer(outPktHdr, outMailHdr, SC_ServerReply_DestroyCondition, requestID, machineID,
										  mailbox, 0);
					} else {
						sendReplyToServer(outPktHdr, outMailHdr, SC_ServerReply_DestroyCondition, requestID, machineID,
										  mailbox, 1);
				...
				case SC_Server_Acquire: {
					...
					//If it's not in our indexes, we don't have it, so reply no
					if (lockNum / 100 != myMachineID) {
						sendReplyToServer(outPktHdr, outMailHdr, SC_ServerReply_Acquire, requestID, machineID, mailbox,
										  0);
					} else {
						sendReplyToServer(outPktHdr, outMailHdr, SC_ServerReply_Acquire, requestID, machineID, mailbox,
										  1);
				...
				case SC_Server_Signal1: {
					...
					//If it's not in our indexes, we don't have it, so reply no
					if (cvNum / 100 != myMachineID) {
						sendReplyToServer(outPktHdr, outMailHdr, SC_ServerReply_Signal1, requestID, machineID, mailbox,
										  0);
					} else {
						sendReplyToServer(outPktHdr, outMailHdr, SC_ServerReply_Signal1, requestID, machineID, mailbox,
										  1);
				...
				case SC_Server_Signal2: {
					...
					if (lockNum / 100 != myMachineID) {
						sendReplyToServer(outPktHdr, outMailHdr, SC_ServerReply_Signal2, requestID, machineID, mailbox,
										  0);
					} else {
						lockNum = lockNum % 100; //grab lock index
				...
				case SC_Server_Signal3: {
					...
					if (foundLock && badLock) {
						sendReplyToServer(outPktHdr, outMailHdr, SC_ServerReply_Signal3, requestID, machineID, mailbox,
										  -1);
				...
				case SC_Server_Wait1: {
					...
					//If it's not in our indexes, we don't have it, so reply no
					if (cvNum / 100 != myMachineID) {
						sendReplyToServer(outPktHdr, outMailHdr, SC_ServerReply_Wait1, requestID, machineID, mailbox,
										  0);
					} else {
						cvNum = cvNum % 100;
						//Validate user input: send -1 if bad
				...
				case SC_Server_Wait2: {
					...
					//If it's not in our indexes, we don't have it, so reply no
					if (lockNum / 100 != myMachineID) {
						sendReplyToServer(outPktHdr, outMailHdr, SC_ServerReply_Wait2, requestID, machineID, mailbox,
										  0);
					} else {
						int lockID = lockNum;
						lockNum = lockNum % 100;
				...
				case SC_Server_CreateMV: {
					...
					//If doesn't, send a reply of NO to server that made the request
					if (existingMVID == -1) {
						sendReplyToServer(outPktHdr, outMailHdr, SC_ServerReply_CreateMV, requestID, machineID, mailbox,
										  0);
					}
				...
				case SC_Server_SetMV: {
					...
					//If it's not in our indexes, we don't have it, so reply no
					if (mvNum / 100 != myMachineID) {
						sendReplyToServer(outPktHdr, outMailHdr, SC_ServerReply_SetMV, requestID, machineID, mailbox,
										  0);
					} else {
						sendReplyToServer(outPktHdr, outMailHdr, SC_ServerReply_SetMV, requestID, machineID, mailbox,
										  1);
				...
				case SC_Server_GetMV: {
					...
					//If it's not in our indexes, we don't have it, so reply no
					if (mvNum / 100 != myMachineID) {
						sendReplyToServer(outPktHdr, outMailHdr, SC_ServerReply_GetMV, requestID, machineID, mailbox,
										  0);
					} else {
						sendReplyToServer(outPktHdr, outMailHdr, SC_ServerReply_GetMV, requestID, machineID, mailbox,
										  1);
				...
				case SC_ServerReply_Wait3: {
					...
					if (!yes) {
						...
						if (reply == 0) {
							...
							sendReplyToServer(currentRequest->replyServerMachineID, currentRequest->replyServerMailbox,
